...
Commit close-out docs + moved file together using Conventional Commits.

Do not merge into `dev` locally.

Push branch and create a Pull Request targeting `dev`.

If the issue is a follow-up on an existing open Pull Request for the same problem, push new commits to the same branch and continue using that existing Pull Request (do not create a new Pull Request).
